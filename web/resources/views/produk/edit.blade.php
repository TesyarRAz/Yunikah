...
<div class="container h-100">
    <div class="row align-items-center h-100">
        <div class="col-md-5 col-12 mx-auto">
            <div class="card p-3">
                <h2 align="center">Edit Data {{ ucfirst($produk->name) }}</h2>

                <form enctype="multipart/form-data" class="form-group my-5" action="{{ route('produk.update', [$mitra->id, $produk->id]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <input id="harga" type="number" class="form-control @error('harga') is-invalid @enderror" name="harga" value="{{ old('harga') ?? $produk->harga }}" required autocomplete="harga" autofocus>
                        ...
                        <div class="col-md-6">
                            <select class="form-control @error('kategori_id') is-invalid @enderror" id="kategori_id" name="kategori_id">
                                @foreach ($kategori as $m)
                                    <option value="{{ $m->id }}" {{ (@old('kategori_id') ?? $produk->kategori_id) == $m->id ? 'selected' : ''}}>{{ $m->label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            <select class="form-control @error('type') is-invalid @enderror" id="type" name="type">
                                @foreach (['custom', 'tersedia', 'combo'] as $type)
                                    <option value="{{ $type }}" {{ (@old('type') ?? $produk->type) == $type ? 'selected' : ''}}>{{ ucfirst($type) }}</option>
                                @endforeach
                            </select>
